.PHONY: help clean install dev-install test test-all test-cov lint format check build sdist wheel docs html clean-docs test-rtd upload upload-test check-package
...
lint:
	flake8 serilux/ tests/ examples/ --max-line-length=100 --extend-ignore=E203,W503,E501,E731

format:
	black serilux/ tests/ examples/

format-check:
	black --check serilux/ tests/ examples/
